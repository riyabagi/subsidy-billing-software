                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            // echo "<td>{$row['ndp']}</td>";
                            // echo "<td>{$row['subsidyreceived']}</td>";
                            ...
                            echo "</td>";
                            echo '<td>';
                            echo '<i class="fa-regular fa-pen-to-square" style="color: green; margin-right: 20px; cursor: pointer;" data-id="' . $row['billno'] . '" onclick="editRecord(' . $row['billno'] . ')"></i>';
                            echo '<i id="deleteIcon_' . $row['billno'] . '" class="fa-solid fa-trash-can" style="color: #e01a1a;margin-right: 20px; cursor: pointer;" data-id="' . $row['billno'] . '" onclick="deleteRecord(' . $row['billno'] . ')"></i>';
                            echo '<i class="fa-solid fa-file-invoice" style="color: #007bff; cursor: pointer;" onclick="generateInvoice(' . $row['billno'] . ')"></i>'; // New icon for Generate Invoice
                            echo '</td>';
                            echo "</tr>";
                ...
                <script>
                    document.addEventListener("DOMContentLoaded", function () {
                        ...
                        function exportToExcel() {
                            ...
                            // Clone the table to remove the "Actions" column before converting to sheet
                        ...
                        // Add the event listener to your table rows for navigation
                    ...
                    function editRecord(id) {
                        // Redirect to the details page with the record ID
                        window.location.href = "billdetails.php?id=" + id;
                    }
                    ...
                    function deleteRecord(id) {
                        if (confirm("Are you sure you want to delete this record?")) {
                            ...
                            $.ajax({
                                url: './ajax/delete_recordbillout.php',
                                method: "POST",
                                data: {
                                    id: id
                                },
                    ...
                    function generateInvoice(id) {
                        // Redirect to the invoice page with the record ID
