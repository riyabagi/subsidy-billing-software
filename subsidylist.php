    ...
    <main class="table" id="subsidy_table">
        ...
        <section class="table_body">
            <table>
                ...
                <tbody>
                    <?php
                    // Fetch data from the subsidy table
                    $sql = "SELECT * FROM subsidy";
                    $result = $con->query($sql);
                    $counter = 0;
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            // Set the URL for the details page
                            // $detailsURL = "details.php?id=" . $row['sno'];
                    
                            // Wrap each row with an <a> tag
                            ...
                            echo '<i id="deleteIcon_' . $row['sno'] . '" class="fa-solid fa-trash-can" style="color: #e01a1a; cursor: pointer;" data-id="' . $row['sno'] . '" onclick="deleteRecord(' . $row['sno'] . ')"></i>';
                            echo '</td>';
                            echo "</tr>";
