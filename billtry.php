    ...
    <div class="container" id="container">
        ...
        <table id="itemTable">
            ...
            <tbody>
                ...
                <tr class="row-style">
                    <td><input type="text" name="item[]" placeholder="Particulars" oninput="myFunction13(this)"></td>
        ...
        $sqlProduct = "SELECT particulars FROM stocks ";
        ...
        while ($row = mysqli_fetch_assoc($resultProduct)) {
            ...
            $particularsArray[] = $particulars;
            ?>

            <script>
                var phpParticularsArray = <?php echo json_encode($particularsArray); ?>;

                function myFunction13(inputElement) {
                    var row = inputElement.closest('tr');
                    if (row) {
                        var particularsInput = row.querySelector('input[name="item[]"]');
                        console.log(particularsInput);
                        if (particularsInput) {
                            var cropValue = inputElement.value;

                            if (cropValue.trim() === '') {
                                document.getElementById('cropDisplay').innerText = '';
                                return;
                            }

                            var filteredParticulars = phpParticularsArray.filter(part => part.startsWith(cropValue));

                            var dataList = document.getElementById('particularsList');
                            if (!dataList) {
                            ...
                            filteredParticulars.forEach(particular => {
                            ...
                            var cropDisplay = document.getElementById('cropDisplay');
                            if (cropDisplay) {
                                cropDisplay.innerText = filteredParticulars.join(', ');
                            }
                        }
                    }
                }
        ...
        <script>
            $(document).ready(function () {
                $("#save").click(function () {
                    ...
                    if (name == '') {
                        ...
                        $.ajax({
                            url: './ajax/billoutwordajaxtry.php',
                            method: "POST",
                            data: {
                                billDate: billDate,
                                name: name,
            ...
            //             $.ajax({
            //                 url: './ajax/stockautocomplete.php',
            //                 method: 'POST',
            //                 dataType: 'json',
            //                 data: {
            //                     term: request.term
            ...
            //                 },
            //                 error: function (err) {
            //                     console.error('Error in Autocomplete request', err);
            //                 }
            //             });
            //         },
            //         minLength: 1
